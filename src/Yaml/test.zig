...
test "bools" {
    ...
    const arr = try yaml.parse(arena.allocator(), [2]bool);
    try testing.expectEqualSlices(bool, &[_]bool{ false, true }, &arr);
}
...
test "enums" {
    ...
    try testing.expectEqualSlices(TestEnum, &[_]TestEnum{ .alpha, .beta, .charlie }, &arr);
}
...
test "stringify an enum" {
    ...
    try testStringify("beta", TestEnum.beta);
