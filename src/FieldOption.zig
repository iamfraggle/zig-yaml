...
pub const ParseCallback = *const fn (type, *anyopaque, anytype, Allocator) callconv(.@"inline") anyerror!void;
...
pub inline fn parseArrayList(comptime T: type, target: *anyopaque, raw: anytype, alloc: Allocator) anyerror!void {
    const slice: T = raw;
