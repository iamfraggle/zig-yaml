...
fn parseValue(self: Yaml, arena: Allocator, comptime T: type, target: *T, value: Value, comptime options: ?[]const FieldOption) Error!void {
    switch (@typeInfo(T)) {
        ...
        .@"struct" => { try self.parseStruct(arena, T, target, value.asMap() orelse return error.TypeMismatch, options); },
        .@"union" => { try self.parseUnion(arena, T, target, value, options); },
        .array => { try self.parseArray(arena, T, target, value.asList() orelse return error.TypeMismatch, options); },
        .pointer => if (value.asList()) |list| {
            try self.parsePointer(arena, T, target, .{ .list = list }, options);
        } else if (value.asScalar()) |scalar| {
            return self.parsePointer(arena, T, target, .{ .scalar = try arena.dupe(u8, scalar) }, options);
        } else if (value.asMap()) |map| {
            return self.parsePointer(arena, T, target, .{ .map = map }, options);
        } else return error.TypeMismatch,
        .@"enum" => target.* = try self.parseEnum(T, value),
    ...
    inline for (struct_info.fields) |field| {
        ...
        if (value) |v| {
            ...
            if (settings != null and settings.?.parse_cb != null) {
                ...
                cb(field_type, &@field(target, field.name), val, arena) catch return Error.Unimplemented; // TODO - add a new error type?
                arena.free(val);
            }
    ...
    switch (ptr_info.size) {
        .slice => {
            if (ptr_info.child == u8) {
                target.* = try arena.dupe(u8, value.asScalar() orelse return Error.TypeMismatch);
                return;
            }
        ...
        .one => {
            ...
            try self.parseValue(arena, ptr_info.child, parsed, value, options);
            target.* = parsed;
        },
        else => return error.Unimplemented,
    }
